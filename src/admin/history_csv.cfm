<cfsetting showDebugOutput = "no" ><cfscript>
	...
	r = queryExecute("
		...
		WHERE (p.name = 'completed' OR p.name IS NULL) AND (b.uid = @uid OR @uid IS NULL)
		ORDER BY r.uid
...
</cfscript><cfsavecontent variable="csvFile"><cfoutput>UID,SAMPLE_CODE,LOGIN,GROUP_ASSIGNMENT,COMPLETED,LOGIN_CREATED,FIRST_LOGIN,PAGE,AUDIO_FILE,LABEL,POSITION,ANSWERED,FINALIZED
<cfloop query="#r#">"#r.uid#","#r.sample_code#","#r.login#","#r.group_assignment#","#r.completed#","#dateFormat(r.created,"m/d/yy")# #timeFormat(r.created,"hh:mm")#","#dateFormat(r.first_login,"m/d/yy")# #timeFormat(r.first_login,"hh:mm")#","#r.page#","#r.audio#","#r.label#","#r.position#","#dateFormat(r.added,"m/d/yy")# #timeFormat(r.added,"hh:mm")#","#r.finalized#"
